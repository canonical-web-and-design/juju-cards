...
let jujuCards = (demoDomain) => {
  ...
  let renderBundle = (card, data) => {
    ...
    const deployTitle = getButtonText(
      demoDomain,
      getImageID(id),
      card.dataset.dd
    );
    ...
    let dom =
      `<div class="juju-card__container bundle-card">` +
      ...
      `<div class="bundle-card__actions-command">` +
  ...
  let renderCharm = (card, data) => {
    ...
    let dom =
      `<div class="juju-card__container charm-card">` +
      ...
      `<div class="charm-card__actions-command">` +
      `<input class="charm-card__actions-field js-clipboard-input" readonly="readonly" value="juju deploy ${id}" onclick="this.select();" id="cli-deploy">` +
